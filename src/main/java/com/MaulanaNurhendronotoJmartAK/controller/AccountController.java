...
public class AccountController implements BasicGetController<Account>{
	...
	public Account Register(@RequestParam String name, @RequestParam String email, @RequestParam String password) {
		...
		boolean passwordCheck = REGEX_PATTERN_PASSWORD.matcher(password).find();
		...
		if(!name.isBlank() && emailCheck && passwordCheck && accountTable.stream().noneMatch(account -> account.email.equals(email))) {
